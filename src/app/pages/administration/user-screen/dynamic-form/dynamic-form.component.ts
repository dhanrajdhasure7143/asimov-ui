...
export class DynamicFormComponent implements OnInit {
  ...
  constructor(private rest: RestApiService, private datatransfer: DataTransferService) { }

  ngOnInit(): void {
    console.log("inputFieldData",this.inputFieldData,this.details,this.dash_board_list);
    if(this.details==undefined){
      this.isEditForm =false;
      this.inputFieldData.forEach((item: any) => {
        this.generatedForm.addControl(item.ColumnName, new FormControl(item.DefaultValue))
      })
    }else{
      this.isEditForm =true;
      this.inputFieldData.forEach((item: any) => {
        if(this.details[item.ColumnName]!=undefined || this.details[item.ColumnName] != '')
           this.generatedForm.addControl(item.ColumnName, new FormControl(this.details[item.ColumnName]))
        else
           this.generatedForm.addControl(item.ColumnName, new FormControl(item.DefaultValue))
