...
addComingSoonAgent(name, description) {
  const comingSoonAgent = {
      "name": name,
      "description": description,
      "is_coming_soon": true
  };

  this.unsubscribed_agents.push(comingSoonAgent);
}
...
proceedToSubscribe() {
  ...
  navigateToPurchaseAgent(plan){
    ...
    if(plan.name =="RFP"){
      this.router.navigate(['/pages/aiagent/subscription/rfp'], { queryParams: { token: emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return
      }
    if(plan.name =="Developer Agent"){
      this.router.navigate(['/pages/aiagent/subscription/dev'], { queryParams: { token: emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return
    }
    if(plan.name =="Recruitment"){
      this.router.navigate(['/pages/aiagent/subscription/recruitment'], { queryParams: { token: emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return
    }
    if(plan.name == "Customer Support"){
      this.router.navigate(['/pages/aiagent/subscription/chatbot'], { queryParams: { token: emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return
    }
    if(plan.name =="Testing Agent"){
      this.router.navigate(['/pages/aiagent/subscription/testing'], { queryParams: { token:emailToken, id:plan.id, isYearly:plan.selectedTire =='Monthly'?'false':'true' } });
      return
    }
