  ...
  ngOnInit() {
    ...
    for (let f of this.fields) {
      if (f.type == 'email')
        ...
        fieldsCtrls[f.name + '_' + f.id] = new FormControl(f.value || '', f.required && f.dependency == '' ? [Validators.compose([Validators.required])] : [])
    }
    //Validators.pattern("^[A-Za-z0-9]+[A-Za-z0-9-@.\/#&+\w\s_?!,$~| =()\\{\\}\\[\\]]*$")
