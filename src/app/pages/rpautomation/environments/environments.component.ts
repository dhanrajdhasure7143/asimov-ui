...
@Component({
  ...
  export class EnvironmentsComponent implements OnDestroy, OnInit{
 ...
 async getallData()
  ...
  async deleteProducts(){
		...
		const selectedProducts = this.environments.filter(product => product.checked).map(p => p.environmentId);
    await this.environmentservice.deleteenvironment(selectedProducts).subscribe( res =>{ 
      alert(res);  
      this.chanref.detectChanges();
      this.rerender();
  ...
  checktoupdate()
  {
    ...
    if(selectedProducts.length==1){
      ...
      this.updateid=selectedProducts[0];
    }else
    {
      this.updateflag=false;
    }
