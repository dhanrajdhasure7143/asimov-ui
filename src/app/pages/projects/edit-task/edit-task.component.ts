...
export class EditTaskComponent implements OnInit {
  ...
  OnChangeTaskCategory(data) {
    if (this.selectedtask.taskCategory == data.target.value) {
      this.updatetaskForm
        .get("correlationID")
        .setValue(this.selectedtask.correlationID);
    } else {
  ...
  updatetask() {
    if (this.updatetaskForm.valid) {
      ...
      this.spinner.show();
      this.rest.updateTask(taskupdatFormValue).subscribe(
        (res) => {
          this.spinner.hide();
          let status: any = res;
  ...
  getImage() {
    ...
    this.rest.getUserDetails(userid).subscribe((res) => {
      ...
      if (
        this.retrieveResonse.image == null ||
  ...
  getTaskAttachments() {
    this.rest
      .getTaskAttachments(this.selectedtask.projectId, this.selectedtask.id)
      .subscribe((data) => {
  ...
  getUserRole() {
    let user = this.users_list.find(
      (item) => item.userId.userId == this.taskresource
    );
  ...
  updatecomment(id) {
    ...
    for (let i = 0; i < this.taskcomments.length; i++) {
      if (this.taskcomments[i].id == id) {
        this.taskcomments[i].comments =
          this.updatetaskForm.get("editcomment").value;
      }
    }
  }
  ...
  onDownloadItem(fileName) {
    ...
    this.rest.downloadTaskAttachment(data).subscribe((data) => {
      ...
      link.href =
        ...
        extension == "svg" ||
        extension == "gif"
          ? `data:image/${extension};base64,${response[0]}`
          : `data:application/${extension};base64,${response[0]}`;
      link.click();
      this.spinner.hide();
    });
    this.getTaskAttachments();
  ...
  onDeleteItem(id, fileName) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.spinner.show();
        this.rest.deleteFiles(input).subscribe(
          (res) => {
            this.spinner.hide();
            let status: any = res;
            if (status.errorMessage == undefined) {
  ...
  filecheckAll(ev) {
    if (this.filecheckeddisabled == false)
      this.taskattacments = this.taskattacments.map((item) => {
        item.checked = true;
        return item;
      });
  ...
  checktodelete() {
    this.taskattacments.filter((item) => item.checked == true).length > 0
      ? (this.filecheckflag = false)
      : (this.filecheckflag = true);
    this.taskattacments.filter((item) => item.checked == true).length ==
    this.taskattacments.length
  ...
  filechecktoggle(id, event) {
    this.taskattacments.find((data) => data.id == id).checked =
  ...
  onDownloadSelectedItems() {
    ...
    downloadSelectedfiles = this.taskattacments
      .filter((product) => product.checked == true)
      .map((p) => {
        return p.fileName;
      });
    this.spinner.show();
    this.rest.downloadTaskAttachment(downloadSelectedfiles).subscribe(
      (response: any) => {
        this.spinner.hide();
        if (response.errorMessage == undefined) {
          var zip = new JSZip();
          response.forEach((value, i) => {
            let extension = downloadSelectedfiles[i]
              .toString()
              .split("")
              .reverse()
              .join("")
            ...
            else zip.file(downloadSelectedfiles[i], value);
          });
  ...
  onDeleteSelectedItems(event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.spinner.show();
        this.rest.deleteFiles(selectedFiles).subscribe(
          (res) => {
            let status: any = res;
            this.spinner.hide();
            if (status.errorMessage == undefined) {
  ...
  getallbpmprocess() {
    this.rest.getprocessnames().subscribe((data) => {
      ...
      this.bpm_process_list = resp.sort((a, b) =>
  ...
  getallpiprocess() {
    this.rest.getAlluserProcessPiIds().subscribe((data) => {
      let response: any = data;
      this.pi_process_list = response.data.sort((a, b) =>
        a.piName.toLowerCase() > b.piName.toLowerCase()
          ? 1
          : b.piName.toLowerCase() > a.piName.toLowerCase()
          ? -1
          : 0
      );
    });
  ...
  getallbots() {
    this.rest.getAllActiveBots().subscribe((data) => {
      let response: any = data;
      this.bot_list = response.sort((a, b) =>
        a.botName.toLowerCase() > b.botName.toLowerCase()
          ? 1
          : b.botName.toLowerCase() > a.botName.toLowerCase()
          ? -1
          : 0
      );
    });
  ...
  getTaskCategoriesByProject() {
    this.rest
      .getTaskCategoriesByProject(this.project_id)
      .subscribe((res: any) => {
