...
export class BusinessProcessComponent implements AfterViewChecked {
  ...
  constructor(private router: Router, private activatedRoute: ActivatedRoute, private cdRef: ChangeDetectorRef, 
    private dt: DataTransferService,private rest:RestApiService, 
  ...
  ngAfterViewChecked() {
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      localStorage.setItem("navigateTo", params.navigateTo);
 ...
 deployNotation(){
  ...
  backtoNavigate(){
    if(localStorage.getItem('projectId')!="null" && localStorage.getItem('projectId') != undefined ){
  ...
  saveandSubmit(){
    ...
    if(this.isShowConformance){
      if(!this.process_owner){
        this.toastService.showError(this.toastMessages.SlctPownerErr);
        return;
      }
