...
export class SoIncidentManagementComponent implements OnInit {
  ...
  seachInput:string="";
  @ViewChild("incidentTablePaginator",{static:false}) incidentTablePaginator: MatPaginator;
  @ViewChild("incidentTableSort",{static:false}) incidentTableSort: MatSort;
  incidentTableDisplayedColumns: any[] = ["incidentId","convertedCreatedTime","assignedTo","description","incidentStatus"];
  ...
  {
    this.spinner.show();
    this.rest.getIncident().subscribe((response:any)=>{
      ...
      if(response.errorMessage==undefined)
      {
        ...
        if(response.configuration==true)
        {
          let modifiedResponse=response.incidents.map((item:any)=>{
            item['convertedCreatedTime']=moment(item.createdAt).format("MMM, DD, yyyy, hh:mm A")
            return item;
          })
          setTimeout(()=>{
            this.incidentTableDataSource = new MatTableDataSource(modifiedResponse);
